...
public:
    int garbageCollection(vector<string>& garbage, vector<int>& travel) {
        ...
        bool fPaper = false;
        bool fGarbg = false;
        // loop from the end first so we can tell if a truck even needs to go that far (bool vals)
        for(int i = garbage.size()-1; i >= 0; i--) {
            for(int j = 0; j < garbage[i].length(); j++) {
                // add one minute for each piece of garbage to the respective truck
                // mark found to add travel time later
                if(garbage[i][j] == 'M'){
                    fMetal = true;
                    tMetal++;
            ...
            // add travel time if you've seen the respective type of garbage (ignore first stop)
            if(i > 0){
                if(fMetal) tMetal += travel[i-1];
                if(fPaper) tPaper += travel[i-1];
                if(fGarbg) tGarbg += travel[i-1];
            }
        }
        return (tMetal + tPaper + tGarbg);
    }
};

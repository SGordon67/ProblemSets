...
public:
    int minOperations(int n) {
        // get the median, since all numbers will go to median
        int median = 0;
        if(n%2){
            median = n+1;
        } else{
        ...
        // get the number of total operations we must do and sum them up
        int operations = 0;
        for(int i = 0; i < n; i++){
            operations  += abs(median - ((i*2)+1));
        }

        // since we can do two operations at once, divide by 2 and return
        return operations / 2;
    }
};
